...
class MusicPlayer {
  constructor(suffix = '') {
    ...
    this.virtualScrolling = {
      itemHeight: 60, // Approximate height of each li item
      containerHeight: 300, // Height of the scrollable container
      visibleItems: 25, // Max items to show at once
      startIndex: 0,
      endIndex: 25,
      scrollTop: 0,
      totalHeight: 0
  ...
  handleScroll() {
    ...
    const endIndex = Math.min(
      startIndex + this.virtualScrolling.visibleItems,
      this.currentMusicArray.length
    );
    
    // Only update if the visible range has changed significantly
    if (Math.abs(startIndex - this.virtualScrolling.startIndex) > 5) {
  ...
  populateMusicList(musicArray) {
    ...
    this.virtualScrolling.totalHeight = musicArray.length * this.virtualScrolling.itemHeight;
    ...
    scrollContainer.style.height = `${this.virtualScrolling.totalHeight}px`;
    scrollContainer.style.position = 'relative';
    
    // Create viewport for visible items
    const viewport = document.createElement('div');
    viewport.id = `viewport${this.suffix}`;
    viewport.style.position = 'absolute';
    viewport.style.top = '0px';
    viewport.style.width = '100%';
    
    scrollContainer.appendChild(viewport);
    this.ulTag.appendChild(scrollContainer);
    
    // Initialize with first batch of items
    this.virtualScrolling.startIndex = 0;
    this.virtualScrolling.endIndex = Math.min(this.virtualScrolling.visibleItems, musicArray.length);
  ...
  updateVisibleItems() {
    ...
    const viewport = this.ulTag.querySelector(`#viewport${this.suffix}`);
    if (!viewport) return;
    
    const { startIndex, endIndex } = this.virtualScrolling;
    const visibleItems = this.currentMusicArray.slice(startIndex, endIndex);
    ...
    visibleItems.forEach((music, i) => {
      const actualIndex = startIndex + i;
      const liTag = this.createListItem(music, actualIndex);
      
      // Position the li element
      liTag.style.position = 'absolute';
      liTag.style.top = `${actualIndex * this.virtualScrolling.itemHeight}px`;
      liTag.style.width = '100%';
      liTag.style.height = `${this.virtualScrolling.itemHeight}px`;
  ...
  createListItem(music, actualIndex) {
  ...
  updatePlayingSong() {
    ...
    const allLiTags = viewport.querySelectorAll("li");
  ...
  scrollToSongByIndex(targetIndex) {
    // Only auto-scroll if in normal mode
    if (this.isShuffleMode || !this.ulTag || !this.repeatBtn) {
      return;
    }
    ...
    const targetScrollTop = targetIndex * this.virtualScrolling.itemHeight;
    const containerHeight = this.ulTag.clientHeight;
    
    // Center the target item in the viewport
    const centeredScrollTop = targetScrollTop - (containerHeight / 2) + (this.virtualScrolling.itemHeight / 2);
    const maxScroll = this.virtualScrolling.totalHeight - containerHeight;
    const finalScrollPosition = Math.max(0, Math.min(centeredScrollTop, maxScroll));
